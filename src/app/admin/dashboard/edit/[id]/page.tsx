...
export default function EditPortfolioPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const fetchItem = async () => {
      try {
        ...
        if (data.success && data.data) {
          ...
          let extras: string[] = [];
          try {
            extras = typeof item.additionalImages === "string"
          ...
          setError("Data portofolio tidak ditemukan.");
        }
      } catch (err) {
        console.error(err);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        throw new Error(data.message || "Gagal mengembalikan data.");
      }
  ...
  if (fetching) {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
        <div className="text-slate-500 text-sm font-medium">Memuat data portofolio...</div>
      </div>
    );
  }
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <form onSubmit={handleSubmit} className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm space-y-8">
          <div className="border-b border-slate-100 pb-6">
            <h1 className="text-2xl font-black text-slate-900">Edit Portofolio</h1>
            <p className="text-xs text-slate-500 mt-1">
              Perbarui informasi atau gambar pendukung untuk portofolio ini.
            </p>
          </div>
